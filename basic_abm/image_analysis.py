...
cols = ["Tag number","Cluster size", "Cluster volume", "Cluster Centre x", "Cluster Centre y", 
           "Event", "Clusters in event", "Timestep"]
...
for i in range(len(time_list)):
    ...
    array_index_current_time, num_clus = label(array_binary)
    area_2D_current = []
    for j in range(1,array_index_current_time.max() + 1):
        area_2D_current.append(sum(array_index_current_time==j))
    # area_2D_current = sum(array_binary, array_index_current_time, index=arange(array_index_current_time.max() + 1))
